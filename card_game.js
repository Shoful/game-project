...
function buildDeck() {
  ...
  for (let i = 0; i < ranks.length; i++) {
    for (let j = 0; s < suits.length; j++) {
      deck.push({ "suit": suits[s], "rank": [i], "value": i })
    }
  }
  return deck;
}
...
function shuffle(deck) {
  ...
  while (currentIndex !== 0) {
    ...
    // This allows the current value to be placed in a variable which is temporary.
    temporaryValue = shuffledDeck[currentIndex];
    // This allows for the current value to be replaced by a random value.
    shuffledDeck[currentIndex] = shuffledDeck[randomIndex];
    // The random value is then exchanged for the temporary value. 
...
function guess(card1, card2) {
  ...
  let input = getInput('Will the next card will be higher type "h".  If lower type "l".');
  ...
  if (input === "h") {
    ...
    getInput('Sorry, guess again. No points in this round. Try again by typing only either "h" or "l".');
    return false;
  }
}
...
function playGame() {
  ...
  while (score < 5 && score < deck.length) {
    ...
    if (guess(currentCard, nextCard) == true) {
      ...
      console.log('You were wrong and got no points');
    }
    currentCard = nextCard;
  }
  if (deck.length === 0) {
    console.log('You are out of cards and have lost.')
  } else {
    console.log('You won!');
  }
}
